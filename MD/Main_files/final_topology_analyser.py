...
#Function that splits our batches into four
def split_into_four_groups(nums, files):
    N = len(nums)
    overall_avg = sum(nums) / N
    indexed_nums_files = list(enumerate(zip(nums, files)))
    sorted_indexed_nums_files = sorted(indexed_nums_files, key=lambda x: x[1][0], reverse=True)
    
    # Initialize the four groups
    groups_nums = [[] for _ in range(4)]
    groups_files = [[] for _ in range(4)]
    group_sums = [0] * 4
    group_counts = [0] * 4
    target_size = N // 4
    
    # Use a min-heap to keep track of the group with the smallest sum and least elements
    heap = [(0, 0), (0, 1), (0, 2), (0, 3)]
    
    for index, (num, file) in sorted_indexed_nums_files:
        # Get the group with the smallest sum and least elements
        group_sum, group_index = heapq.heappop(heap)
        
        # Assign the number, its index, and file to this group
        groups_nums[group_index].append((index, num))
        groups_files[group_index].append(file)
        group_sums[group_index] += num
        group_counts[group_index] += 1
        
        # Push the updated group back into the heap
        heapq.heappush(heap, (group_sums[group_index], group_index))
        
        # Ensure that each group doesn't exceed the target size
        if group_counts[group_index] >= target_size:
            heap = [(s, i) for s, i in heap if i != group_index]
            heapq.heapify(heap)
    
    return groups_nums, groups_files
...
#arguments = sys.argv
#if len(arguments) != 3:
#    print("Need two arguments start and end batch")

#batch_start=int(arguments[1])
#batches_num=int(arguments[2])
#batch_end=batch_start+batches_num-1




#Splitting batches into 4 groups so that they roughly have the same average size 
pattern = './MD_runs/batches/batch*/addition_details.dat'
files = np.array(glob.glob(pattern))

nns=[] #Monomers added in each batch i.e. AB_2 monomers
for name in files:
    data_temp=np.loadtxt(name)
  #  print("Length of monomers added" +str(len(data_temp)))
   # if len(data_temp)==0:
   #     print(name)
    nns.append(len(data_temp))



# Example usage:
groups_nums, groups_files = split_into_four_groups(nns, files)

nn_groups=[]
nam_groups=[]


for i, (group_nums, group_files) in enumerate(zip(groups_nums, groups_files)):
    values = [num for idx, num in group_nums]
    nn_groups.append(np.array(values))
    nam_groups.append(np.array(group_files))
    size=np.array(values)
...
#Randomising data so that each cpu takes in roughly similar workload  
np.random.seed(1)
for i in range(0,len(nn_groups)):
    nn_groups[i] , nam_groups[i] = unison_shuffled_copies(nn_groups[i], nam_groups[i])
    nam_groups[i]=nam_groups[i].tolist()
    
    for j in range(0,len(nam_groups[i])):
         nam_groups[i][j]=nam_groups[i][j][0:-20]+'final_configuration.gsd'
...
for group_num in range(0,len(nam_groups)):
    
    print("Batch number : " +str(group_num+1))

    fol1='batch_'+str(group_num+1)
    os.mkdir(fol1)
    os.mkdir(fol1+'/hyperbranched_data')
    mol_counter=0 #Used to count polymer molecules
    size_polymer=[]
    size_abmonomer=[]
    num_mol=len(nam_groups[group_num]) #Number of molecules in topology file
    ...
    for batch in nam_groups[group_num]:
