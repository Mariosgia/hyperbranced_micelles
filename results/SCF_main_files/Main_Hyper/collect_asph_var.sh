...
# Directory path
hm=$(pwd)
dirs=$( ls -d phi_runs/chosen*/phi*/asph*/ )
for dir in $dirs; do
  if [ -d "$dir" ]; then
    ...
    for folder in "${dir}/asph"*; do
        if [ -d "${folder}" ]; then
            ...
            if [ -f "${details_file}" ]; then
                ...
                con_shift=$(sed -n '/con_shift/{s/d0//;s/ .*//;p;q;}' $input_file) #Extract mu_offset
                echo "$con_shift  $edif $con_ene" >> "${output_file}"
            else
