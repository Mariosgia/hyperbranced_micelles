...
module Molecule_Hyper_Copolymer_Analysis
!
  use Global
  use Propagate
  USE OMP_LIB
...
!
  ...
  type hyper_copolymer
    ...
    double precision  , dimension(:,:,:,:), allocatable               :: node_distribution !
...
!            print*, "PS sum", sum(hyper_chains(i)%distribution)*dvol
  ...
  subroutine Calculate_Node(copolymer,propagators)
    ...
    do i=1,copolymer%num_segments !Calculates node distribution based on segment id. linear part is calculated twice. once for one end and another for the other end
...
end module Molecule_Hyper_Copolymer_Analysis
