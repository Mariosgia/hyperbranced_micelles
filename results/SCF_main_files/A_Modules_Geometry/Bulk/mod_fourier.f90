...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!       
!       NOTE THAT THE CODE HAS BEEN MODIFIED TO CREATE A PLAN FOR EACH
!           THREAD IN THE CASE WHERE OPENMP IS USED.
!
!
  use Global
  implicit none
  ...
  integer*8, dimension(:), allocatable  ::  ftplan,ftbackwardplan
  double precision, dimension(:,:,:,:), allocatable  :: ftin ,ftbackwardout 
  double complex, dimension(:,:,:,:), allocatable  :: ftout ,ftbackwardin

  double complex,save, dimension(:,:,:) , allocatable    ::laplace,explaplace
  double complex,save, dimension(:,:,:,:) , allocatable  :: nabla
...
!
  subroutine Initialize_Fourier
    ...
    integer :: th_num,allocStatus

   allocate(ftplan(0:threads-1),stat=allocStatus)
   allocate(ftbackwardplan(0:threads-1),stat=allocStatus)
   allocate(ftin(gridx,gridy,gridz,0:threads-1) ,stat=allocStatus)
   allocate(ftout(gridx/2+1, gridy,gridz,0:threads-1),stat=allocStatus)
   allocate(ftbackwardin(gridx/2+1, gridy,gridz,0:threads-1),stat=allocStatus)
   allocate(ftbackwardout(gridx,gridy,gridz,0:threads-1),stat=allocStatus)

    do th_num=0,threads-1
        call dfftw_plan_dft_r2c_3d(ftplan(th_num),gridx,gridy,gridz,ftin(:,:,:,th_num),ftout(:,:,:,th_num),FFTW_PATIENT)
        call dfftw_plan_dft_c2r_3d(ftbackwardplan(th_num),gridx,gridy,gridz,&
                                        ftbackwardin(:,:,:,th_num),ftbackwardout(:,:,:,th_num),FFTW_PATIENT)
    end do
...
!
  subroutine End_Fourier
    ...
    deallocate(ftin, ftout,ftbackwardin,ftbackwardout)

    do th_num=0,threads-1
        call dfftw_destroy_plan(ftplan(th_num))
        call dfftw_destroy_plan(ftbackwardplan(th_num))
    end do
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
  subroutine Fourier2Real(prop,propfourier,th_num)
...
!
    ftbackwardin(:,:,:,th_num) = propfourier
    call dfftw_execute(ftbackwardplan(th_num))
    prop = ftbackwardout(:,:,:,th_num) 
!
    return
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
  subroutine Real2Fourier(prop,propfourier,th_num)
...
!  
    ftin(:,:,:,th_num)  = prop   
    call dfftw_execute(ftplan(th_num))
    propfourier = ftout(:,:,:,th_num)/(gridx*gridy*gridz)
...
!
    call real2fourier(testfield,testfourier,0)
    call fourier2real(testfieldnew,testfourier,0)
!
     do z=1,gridz
      do y=1,gridy
...
!
  subroutine Calculate_Laplace
    ...
    integer :: x,y,z ,i ,allocStatus
    double precision :: qx,qy,qz
    double precision :: dx,dy,dz



   allocate(explaplace(gridx/2+1,gridy, gridz) ,stat=allocStatus)
   allocate(laplace(gridx/2+1,gridy, gridz) ,stat=allocStatus)
   allocate(nabla(gridx/2+1,gridy, gridz,3) ,stat=allocStatus)
